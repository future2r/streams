...
public final class RandomIntegerGeneratorConfigurationPane extends AbstractConfigurationPane<RandomIntegerGenerator> {
	...
	protected void createContent(final Container contentPane) {
		...
		this.originSpinner = new JSpinner(new SpinnerNumberModel(0, Integer.MIN_VALUE, Integer.MAX_VALUE, 1));
		final var originLabel = new JLabel("Origin (inclusive):");

		this.boundSpinner = new JSpinner(new SpinnerNumberModel(1, Integer.MIN_VALUE, Integer.MAX_VALUE, 1));
	...
	@Override
	protected void initContent(RandomIntegerGenerator operation) {
	...
	@Override
	protected boolean applyContent(final RandomIntegerGenerator operation) {
		operation.setNumber(((Number) this.numberSpinner.getValue()).intValue());
		operation.setOrigin(((Number) this.originSpinner.getValue()).intValue());
		operation.setBound(((Number) this.boundSpinner.getValue()).intValue());
		return true;
	}
}
