...
public final class AllOperationsTest {
	...
	public void testProvider() {
		...
		assertEquals(16, provider.getSourceOperations().size());
		assertEquals(13, provider.getIntermediateOperations().size());
		assertEquals(4, provider.getTerminalOperations().size());

		assertEquals(5, provider.getPresets().size());
	}
